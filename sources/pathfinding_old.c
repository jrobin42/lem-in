...
/*   Updated: 2018/03/26 21:31:40 by jrobin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_path(int *x, int *y, int *path, t_lemin *lemin, int **mat)
{
	...
	*x = -1;
	...
	while (k < i)
	{
	//	ft_printf("%s path[i = %d] = %d path[k = %d] = %d\nused_rooms i = %d et used_rooms k = %d\n\n", (mat[path[i]][path[k]] == 1 ? "LIEN" : "\t"), i, path[i], k, path[k], lemin->used_rooms[path[i]], lemin->used_rooms[path[k]]);
		if (mat[path[i]][path[k]] == 1 && lemin->used_rooms[path[k]] == 0)
		{
			lemin->used_rooms[path[i]] = 0;
			lemin->used_rooms[path[k]] = 1;
			path[++k] = path[i];
	/*		int plop = 0;
			while (plop < lemin->nb_rooms)
			{
				ft_printf("path[plop] = %d\n", path[plop]);
				++plop;
			}
	*/		ft_printf("BELLO !\n");
//			k--;
			...
			{
				ft_printf("reset alliiiiiiiiiiiiiiiiiiiiiii \n");
...
int		pathfinding(t_list **path, t_lemin *lemin, int **mat, int *max_nb_paths)
{
	...
	while (*max_nb_paths)
	{
		...
		int i = 0;
		while (i < lemin->nb_rooms && ((t_path*)(*path)->content)->path[i] != -1)
		{
			ft_printf("path[%d] = %d\n", i, ((t_path*)(*path)->content)->path[i]);
			++i;
		}
		--*max_nb_paths;
		++paths_found;
